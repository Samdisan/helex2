...
function recalculateActDuration($target, $actId, $quests) {
    ...
    $actQuests = array_filter($quests, fn($q) => $q['chapter'] === $target && $q['act_id'] == $actId);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif ($_POST['action'] === 'delete_act') {
        ...
        $quests = array_filter($quests, fn($q) => !($q['chapter'] === $target && $q['act_id'] == $delId));
        saveJson(GAMESTATE_FILE, $state);
...
$currentActQuests = [];
if ($actIndex !== null) {
    $currentActQuests = array_filter($quests, fn($q) => $q['chapter'] === $target && $q['act_id'] == $actIndex);
}
